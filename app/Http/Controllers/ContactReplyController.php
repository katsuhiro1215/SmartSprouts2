<?php

namespace App\Http\Controllers;

use App\Http\Requests\StoreContactReplyRequest;
use App\Http\Requests\UpdateContactReplyRequest;
use App\Models\ContactReply;

class ContactReplyController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        //
    }

    /**
     * Show the form for creating a new resource.
     */
    public function create()
    {
        //
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(StoreContactReplyRequest $request)
    {
        //
    }

    /**
     * Display the specified resource.
     */
    public function show(ContactReply $contactReply)
    {
        //
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(ContactReply $contactReply)
    {
        //
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(UpdateContactReplyRequest $request, ContactReply $contactReply)
    {
        //
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(ContactReply $contactReply)
    {
        //
    }
}
