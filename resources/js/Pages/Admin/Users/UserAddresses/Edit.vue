...
const form = useForm({
  ...
  address2: props.userAddress.address2,
  address3: props.userAddress.address3,
  address4: props.userAddress.address4,
...
const update = async () => {
  ...
  form.post(route('admin.userAddress.update', { user: user.id }), {
    onFinish: () => {
      ...
      <div class="flex sm:flex-row items-center justify-between">
        ...
        <Breadcrumb
          :items="[
            ...
            { name: 'Edit', url: route('admin.userAddress.edit', ['user', 'userAddress']) },
          ]"
        />
      </div>
    </template>
              ...
              <form @submit.prevent="store" class="space-y-6">
                <div class="w-full sm:w-1/2 lg:w-2/3 space-y-4">
                  <div class="flex flex-col gap-4">
                    <InputLabel for="user_id" value="ユーザー名" />
                    <TextInput type="text" v-model="form.username" class="bg-slate-100" readonly />
